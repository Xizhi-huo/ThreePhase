...
任务：Phase 2 前置清理轮 — 删除历史镜像目录与孤立配置（第 17 轮）

你的角色：
你是一位资深 Python 桌面端维护工程师。本轮不做重构、不做功能改动，只做一次窄范围的仓库清理：把第 15 轮意外引入的死代码快照与孤立配置从项目中移除，让 Phase 2 起点干净可审查。

项目背景：
这是一个 PyQt5 三相电并网仿真教学系统，Phase 0 / Phase 1 已全部完成。
Phase 1 第 15 轮的提交（183d25e）意外携带了一份生产代码的镜像快照，落在：
...
这些内容：
- 与真实测试无任何关联（真实测试文件位于 `tests/test_physics_snapshot.py`、`tests/test_assessment_snapshot.py`、`tests/support/stubs.py`、`tests/snapshots/`）。
- 保留的是第 15 轮之前的旧私有句柄命名（`self._hw` / `self._loop_svc` 等），会让未来审查路径出现歧义。
- 已确认全仓无 `from tests.app` / `from tests.services` / `from tests.domain` / `from tests.adapters` 等 import 引用。

本轮唯一主攻目标：
删除上述镜像目录与孤立配置，且只删这些，不做任何其它改动。
...
- §2 基线表
- §3 当前总体进度
- §9 第 15 / 16 轮记录（理解镜像来源）
- §10 下一轮默认起点（Phase 2）

2. 当前 tests/ 目录结构：
       ls tests/
   确认真实测试资产与将被删除的镜像可以一眼区分：
...
2. `git log -1 --stat` 确认起点是第 16 轮提交（Phase 1 正式闭环那次）。
3. 跑基线：
       /Users/promise/opt/anaconda3/envs/power_gui/bin/python -m pytest tests/ -v -p no:cacheprovider
   应 5/5 PASS。
4. 删除前必须做一次独立的引用扫描，确认零引用：
       grep -rn "from tests\.app\|from tests\.services\|from tests\.domain\|from tests\.adapters\|from tests\.docs" --include="*.py" .
       grep -rn "import tests\.app\|import tests\.services\|import tests\.domain\|import tests\.adapters\|import tests\.docs" --include="*.py" .
   两条命令都必须返回空。若非空，暂停并上报，不得擅自删除。
5. 检查 `tests/.claude/settings.json` 是否被任何路径的工作流（CI、脚本、Makefile 等）引用：
       grep -rn "tests/\.claude" --include="*.py" --include="*.yml" --include="*.yaml" --include="*.toml" --include="Makefile" .
   预期为空。若非空，暂停并上报。
...
A. 删除镜像目录
    ...
    git rm -r tests/domain/
    git rm -r tests/adapters/
    git rm -r tests/docs/

B. 删除孤立配置目录
    git rm -r tests/.claude/

C. 清理对应 .pyc 缓存（若 Git 未自动处理）
    查看 `tests/**/__pycache__/` 中是否有残留指向已删除镜像目录的缓存文件。
    如有残留请一并删除。不得误删 `tests/test_physics_snapshot.py` / `tests/test_assessment_snapshot.py` / `tests/support/stubs.py` 所对应的缓存。
...
E. git commit
commit 信息建议：`Phase 2 前置清理(17): 删除历史镜像目录与孤立配置`。

第四步：硬约束（违反即视为越界）

1. 不得改动任何生产代码：`app/`、`services/`、`ui/`、`domain/`、`adapters/`。
2. 不得改动真实测试资产：`tests/test_physics_snapshot.py`、`tests/test_assessment_snapshot.py`、`tests/support/stubs.py`、`tests/snapshots/`、`tests/__init__.py`。
3. 不得做"顺手"的任何代码/格式/import 重排。
4. 不得新增功能文件、不得新增服务类、不得动 `MAINTENANCE_CHECKLIST.md` 以外的非删除目标。
5. 不得启动 Phase 2 的任何工作（不引入 `AssessmentContext`，不改 `assessment_service.py`，不动魔法字符串等）。
6. 不得用 `rm -rf` 等 Git 外的命令删除；必须走 `git rm -r`，以保留删除历史可追溯。
7. 若开工前扫描（第二步 4、5）返回非空，必须停手并如实上报，不得继续删除。

第五步：更新 MAINTENANCE_CHECKLIST.md

- §2 基线表：如行数有微小变化可同步，否则保持不变。
- §3 当前总体进度：保持 "Phase 2 — 评分系统模块化" 不变。
- §9 新增"第 17 轮"记录，严格按 §11 模板：
    - 本轮唯一主攻目标：清理第 15 轮遗留的历史镜像目录与孤立配置
    - 实际完成：列出被删除的目录清单
    - 删除了哪些旧代码：镜像快照（~6900 行）+ `tests/.claude/settings.json`
    - 接口变化：无
    - 耦合度变化：无（纯死代码清理）
    - 快照测试：PASS
    - 回归清单：PASS
    - 下一轮起点：Phase 2 第一步 — 定义 `AssessmentContext` 并切断评分对 ctrl 的依赖
- §10 下一轮默认起点：保持不变（Phase 2 AssessmentContext）。

第六步：完成后输出

1. 被删除的目录 / 文件清单（git diff --stat 摘要即可）。
2. 扫描命令的输出（应为空）。
3. 快照测试结果（5/5 PASS）。
4. `MAINTENANCE_CHECKLIST.md` §9 变更摘要。
5. 明确声明："本轮为非功能性清理；生产代码与真实测试资产零改动。"
