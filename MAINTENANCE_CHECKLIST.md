...
第 18 轮任务提示词：Phase 2-1 — 评分事件常量化 + 建 AssessmentContext 并切断 ctrl 依赖
背景（必读）
MAINTENANCE_CHECKLIST.md 是唯一事实来源，开工前请先完整读一遍 §1 / §2 / §3 / §4 / §9（第 17 轮刚完成 tests/ 历史镜像目录清理）。
当前起点：main 分支最新提交（第 17 轮 Phase 2 前置清理）。Phase 1 已正式闭环，Controller 只剩编排 + 少量 UI 胶水。
现状关键数字：
services/assessment_service.py = 791 行，内含 13 处 self._ctrl 引用（集中在 build_result 的快照兜底读取段）
build_result 的两个调用点：tests/test_assessment_snapshot.py:57, 68 与 services/assessment_coordinator.py:138
快照测试基线：5/5 PASS（python -m pytest tests/ -q）

本轮唯一主攻目标
把 AssessmentService 变成纯函数管道的前置条件：

 1.消除 build_result() 内部所有事件类型魔法字符串，改用 domain/assessment.py 集中常量

 2.定义 AssessmentContext dataclass，把 build_result 当前从 self._ctrl 兜底读取的 6 类数据外部化

 3.把 build_result(session) 改签为 build_result(session, context)；self._ctrl 引用数从 13 降到 1（__init__ 留下以便后续 Phase 2 继续拆分；或降到 0，如可行）

 4.本轮绝对不动评分逻辑本体——所有评分数值、状态、扣分规则保持字节级一致
 
具体工作拆解
第一步：事件常量化（domain/assessment.py）

 1.在 domain/assessment.py 新增 AssessmentEventType 常量类，至少包含：
 ...
 2.开工前用 grep -rn '"<event_name>"' services/ app/ ui/ tests/ 核对每个常量确实是事件类型使用点——不要把参数名/字段名（如 "target"、"layer"、"allowed"、"from_step"）也抽走，它们是 payload 字段而非事件类型

 3.services/assessment_service.py 内 count(...) / all_events(...) / first(...) 的入参全部改为常量引用

 4.如果事件入队侧（assessment_coordinator.append_assessment_event(...)）也用字符串字面量生成这些事件类型，同样改为常量引用；若入队侧用字面量拼装不易抽取，本轮可暂留（仅读侧常量化即可），但需在本轮复盘时如实记录

第二步：定义 AssessmentContext（domain/assessment.py）

 1.新增 @dataclass(frozen=True) 的 AssessmentContext，精确封装 build_result 当前从 ctrl 兜底读取的字段：
 ...
 phase_complete: bool
 pt_exam_complete: bool（= pt_exam_1 AND pt_exam_2）
 closure_complete: bool
 fault_repaired: bool

 2.不允许在本轮把 AssessmentContext 设计得过大（不要一口气塞进 session / events / scene_info —— 那些是 build_result 自有参数，不是 ctrl 依赖）

 3.AssessmentContext.from_snapshot_and_ctrl(snapshot: dict, ctrl) -> AssessmentContext classmethod：把当前 791 行版本 build_result 第 193-211 行的兜底读逻辑完整搬过来，保持与原语义一致

第三步：改签 build_result

 1.签名从 build_result(self, session: AssessmentSession) -> AssessmentResult 改为 build_result(self, session: AssessmentSession, context: AssessmentContext) -> AssessmentResult
 
 2.原 193-211 行的 self._ctrl.xxx 兜底读取段全部删掉，改为读 context.loop_records 等

 3.AssessmentService.__init__ 是否还需要保留 self._ctrl？
如果本轮结束后 self._ctrl 引用数能降到 0：直接把 __init__(self, ctrl) 改为 __init__(self)，并调整 app/main.py:99 的构造调用
如果仍有 1-2 处未迁走（例如评分逻辑里某处对 ctrl 的残留读取），本轮保留 __init__(self, ctrl)，并在轮次记录里如实列出剩余引用点，留到第 19 轮一并处理
优先追求降到 0；但不能为此擅自改动评分逻辑

第四步：更新调用侧
 1.services/assessment_coordinator.py:138 改为：
...
result = self._ctrl.assessment_svc.build_result(session, context)
 ...
 2.tests/test_assessment_snapshot.py:57, 68 两处调用同步更新；若测试用的是完整 state_snapshot，也应通过 AssessmentContext.from_snapshot_and_ctrl 构造，保持与生产路径一致

第五步：更新测试替身
 1.tests/support/stubs.py 已有 loop_svc / pt_voltage_svc / pt_phase_svc / pt_exam_svc / assessment_coord 等公开句柄，AssessmentContext.from_snapshot_and_ctrl 调用时应当无需新增成员；若确有缺失，补齐到 ControllerStub 而非在 AssessmentContext 里做容错兜底

 2.禁止在 AssessmentContext.from_snapshot_and_ctrl 中写 getattr(ctrl, 'xxx', default) 式防御性代码——若字段缺失说明 stub 没补齐，应直接报错

硬约束
 1.绝对不动评分逻辑本体：_build_discipline_scores / _build_step_scores / _build_fault_scores / _build_blackbox_scores（或当前文件里等价的评分域函数）以及它们的 ctx 字典组装代码本轮零改动；修改仅允许出现在 build_result 入口的兜底读段 + 事件计数 lambda 的字面量替换

 2.开工前后两次 python -m pytest tests/ -q 必须均为 5/5 PASS，且 tests/snapshots/ 目录零差异（git diff tests/snapshots/ 必须为空）——评分结果字节级一致是本轮首要安全网

 3.不引入 Optional[AssessmentContext] = None 之类的可选参数回退——这会让整个改动失去意义；context 是必填参数

 4.不在本轮开始拆评分域模块（services/scoring/ 子包留给第 19 轮）；不在本轮动 assessment_service.py 外的评分数值规则
 
 5.不在本轮触碰 ui/ 目录下的评分展示代码（除非必要时的签名适配）

 6.行数预期：services/assessment_service.py 从 791 → 约 770-790 行（不会明显下降，本轮是地基工作）；domain/assessment.py 合理增长

 7.若开工前任何 grep / 替换扫描返回意外结果（例如常量替换时遇到同名字符串但不是事件类型），必须停手并如实上报，不得自作主张改写

 8.不允许自行补充任何 TODO / FIXME 注释——有未决事项应写进轮次记录

交付物

 1.代码变更：domain/assessment.py、services/assessment_service.py、services/assessment_coordinator.py、app/main.py（仅 AssessmentService(self) → AssessmentService() 一行，如降到 0）、tests/test_assessment_snapshot.py、必要时 tests/support/stubs.py

 2.MAINTENANCE_CHECKLIST.md §9 新增「第 18 轮」条目：
主攻目标一句话
实际完成清单（按第一~五步逐项打勾或说明）
self._ctrl 引用数变化（13 → N，如实写）
services/assessment_service.py 行数变化
快照测试结果
下一轮起点：Phase 2-2 — 按评分域拆分为纯函数模块（services/scoring/ 子包）

 3.如 self._ctrl 未能降到 0，在轮次记录中明确列出残留引用点与未迁原因，供第 19 轮接力

 4.提交信息：Phase 2-1(18): 评分事件常量化 + AssessmentContext 建立

开工顺序建议

 1.python -m pytest tests/ -q 取基线（5/5）

 2.先做第一步事件常量化 → 跑测试 → 应仍 5/5

 3.再做第二步定义 Context + from_snapshot_and_ctrl → 不接入，跑测试 → 应仍 5/5

 4.第三步改签 build_result + 第四步调用侧同步 → 跑测试 → 应仍 5/5，且 tests/snapshots/ 无 diff

 5.grep -n 'self._ctrl' services/assessment_service.py 汇报最终引用数

 6.更新 §9 → 提交

复盘问题（必答）

 1.self._ctrl 引用数最终降到多少？若未到 0，剩下的是哪些调用，是否评分逻辑依赖 ctrl 而非 snapshot？

 2.AssessmentContext 字段是否精确对应 build_result 入口段的兜底读？有没有"顺手"加进去但未被使用的字段？

 3.事件常量化之后，domain/assessment.py 是否出现"事件常量"与"现有 dataclass"混放的可读性问题？是否需要单独拆一个子模块？（本轮不必拆，但请给出意见）

 4.第 19 轮拆评分域时，Context 字段是否已经足够？如果发现某个评分域会用到当前 Context 未覆盖的数据，记下来供第 19 轮补齐。
'''
