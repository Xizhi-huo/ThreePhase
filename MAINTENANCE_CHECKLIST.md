...
任务：核查 Phase 1 收尾（第二阶段，第 16 轮）是否正确完成

你的角色：
你是一位资深 Python 桌面端架构审查员。你的任务不是继续开发，而是严格核查这一轮重构是否符合要求、是否存在越界修改、是否保持行为不变。

项目背景：
这是一个 PyQt5 三相电并网仿真教学系统。
Phase 0 已闭环。
Phase 1 前 5 步拆模块已完成。
Phase 1 收尾第一阶段（第 15 轮）已完成 9 个低频服务句柄公开化与 34 个壳清理。
本轮是 Phase 1 正式收尾：只处理以下 3 个高频/全局句柄：
...
本轮目标：
1. 将上述 3 个句柄公开化
2. 将所有调用从 `ctrl.method(...)` 改为 `ctrl.<service>.method(...)`
3. 删除 Controller 中这两组纯转发壳：
   - `FlowModeManager` 相关壳，约 21 个
   - `AssessmentCoordinator` 相关壳，约 11 个
4. `AssessmentService` 本轮只改句柄名，不改内部逻辑，不新增壳删除任务
5. `app/main.py` 的实际行数应按 `502` 计，不是 `419`

你要做的事情只有一件：
严格核查“Phase 1 收尾（第二阶段，第 16 轮）”是否按要求完成。
...
- 是否新增并仅保留：
  - `self.flow_mgr`
  - `self.assessment_coord`
  - `self.assessment_svc`
- 是否不再保留：
  - `self._flow_mgr`
  - `self._assessment_coord`
  - `self._assessment_svc`
- 是否删除了 `FlowModeManager` 相关纯转发壳
- 是否删除了 `AssessmentCoordinator` 相关纯转发壳
- 是否仍保留这些本轮不该删的方法/属性：
...
3. 以下服务文件
重点核查是否已将 `self._ctrl._flow_mgr / _assessment_coord / _assessment_svc` 统一改为公开形式：
...
4. UI 目录下文件
重点核查是否已把调用从 `ctrl.method(...)` 改成 `ctrl.flow_mgr.method(...)` 或 `ctrl.assessment_coord.method(...)`：
- `ui/main_window.py`
- `ui/test_panel.py`
- `ui/panels/*.py`
- `ui/tabs/*.py`
...
重点核查：
- 是否同步把 `_flow_mgr` 改为 `flow_mgr`
- Stub 对外方法签名是否保持不变
- 若 Stub 没有持有 `_assessment_coord / _assessment_svc`，则无需强行增加

6. `tests/test_physics_snapshot.py`
7. `tests/test_assessment_snapshot.py`
本轮不应修改测试逻辑；如存在直接调用 `ctrl.is_assessment_mode()` 等旧入口，需改到公开句柄路径。
...
1. `app/main.py` 中是否已公开：
- `flow_mgr`
- `assessment_coord`
- `assessment_svc`

2. 是否没有保留旧私有句柄作为兼容别名：
- 不允许 `self.flow_mgr = self._flow_mgr = ...`
- 不允许通过 `@property` 做旧名兼容

3. 本轮是否没有改动上一轮已公开的 9 个句柄名称

B. 纯转发壳删除核查
请确认以下两组方法是否已从 Controller 删除，前提是它们确实是纯转发壳：

1. `FlowModeManager` 相关
- `flow_policy`
- `flow_policy_flag`
- `is_teaching_mode`
- `is_engineering_mode`
...
- `require_all_measurements_before_finalize`
- `require_step_pass_to_finalize`
- `should_show_fault_detected_banner`
- `should_show_diagnostic_hints`
...
- `can_inspect_blackbox`
- `can_repair_in_blackbox`
- `should_auto_clear_fault_only_when_all_blackboxes_normal`
- `allow_admin_shortcuts`
- `can_use_pt_exam_quick_record`
- `should_record_assessment_metrics`
- `should_auto_score_assessment`
- `assessment_ends_after_step4_closed_loop`

2. `AssessmentCoordinator` 相关
...
请同时确认：
- 删除总数是否约为 `32`
- `AssessmentService` 本轮没有被误删不存在的“壳”
- 没有误删带实际逻辑的方法
...
- `ctrl.can_advance_with_fault()` -> `ctrl.flow_mgr.can_advance_with_fault()`
- `ctrl.allow_admin_shortcuts()` -> `ctrl.flow_mgr.allow_admin_shortcuts()`

2. Assessment lifecycle 相关
...
- `ctrl.finish_assessment_session_if_ready(...)` -> `ctrl.assessment_coord.finish_assessment_session_if_ready(...)`

3. 服务内部 cross-service 访问
- `self._ctrl._flow_mgr...` -> `self._ctrl.flow_mgr...`
- `self._ctrl._assessment_coord...` -> `self._ctrl.assessment_coord...`
- `self._ctrl._assessment_svc...` -> `self._ctrl.assessment_svc...`
...
1. 没有重写 `AssessmentService.build_result()`
2. 没有引入 `AssessmentContext`
3. 没有改 `domain/` 下任何文件
4. 没有新增新服务类
5. 没有改测试逻辑文件：
...
6. 没有再次改动上一轮 9 个句柄的对外命名
7. 没有顺手做无关的 import/空行/格式大重排
8. 没有修改 README.md / context.md

E. Stub 核查
请确认：

1. `tests/support/stubs.py` 是否只做了最小适配
2. Stub 对外方法签名是否完全未变
3. 是否仅把 `_flow_mgr` 替换为 `flow_mgr`
4. 没有为了贴合真实 Controller 而删除 Stub 便捷方法
...
1. `app/main.py` 的实际行数应按文件真实拆行数计算
2. 本轮后正确值应为 `502`
3. 不能误写成 `419`
4. `MAINTENANCE_CHECKLIST.md` 是否已修正：
- §2 中 `app/main.py` 行数为 `502`
- §9 第 16 轮记录中行数变化为 `483 -> 502`

5. §3 是否已改为：
- `Phase 2 — 评分系统模块化`

6. §4 是否已将：
- `删除 Controller 中已迁出的旧方法` 标为 `[x]`

7. §10 是否已改为：
- `Phase 2 — 定义 AssessmentContext 并切断评分对 ctrl 的依赖`

H. 结果判定核查
请确认是否能支持以下结论：

1. `Phase 1` 已正式完成
2. `Phase 2` 可作为下一轮默认起点
3. `app/main.py` 已显著瘦身，但真实行数应以 `502` 为准，不是 `419`
...
5. 调用点迁移检查
- 明确写是否完成了：
  - `ctrl.method(...) -> ctrl.flow_mgr.method(...)`
  - `ctrl.method(...) -> ctrl.assessment_coord.method(...)`
- 如无法借助 git 判断状态，明确说明“仅完成静态核查”
...
8. 最终判定
- 用一句话给出：
  - “Phase 1 第 16 轮可视为完成，Phase 1 正式闭环”
  - 或
  - “Phase 1 第 16 轮暂不能视为完成”
...
- 不要给大段发散建议
- 只围绕“这一轮是否按要求完成”给出结论
- 优先找“是否遗漏旧调用点”“是否误删非纯壳”“是否误写 419 行数”“是否越界动了 AssessmentService 内部逻辑”
