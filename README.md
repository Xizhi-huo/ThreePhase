...
> 基于 PyQt5 的高压机组并网操作培训桌面应用，模拟隔离母排模式下的完整五步并网流程，包含四个可注入的错误场景供教学使用（E05 / E06 暂时禁用，开发中）。
...
| E04 | ✅ 启用 | PT3 实际变比 11000:93（≈118.28），额定应为 11000:193（≈56.99） | 步骤 2（PT3 二次侧 ≈ 88 V，远低于额定 184 V，标红）/ 步骤 4（PT3 各行压差均偏小） | 无 | recoverable |
| E05 | ⏸ 禁用 | Gen2 过电压 13 kV（AVR 故障） | 步骤 2（PT3 电压超容差）/ 步骤 4（PT3 同相压差偏大）/ 步骤 5（幅值同步失败） | 无 | recoverable |
| E06 | ⏸ 禁用 | Gen2 相角追踪禁用（强制非同期合闸） | 步骤 5（Δθ 不收敛） | 持续警告，强行合闸触发事故 | accident |
...
- **暂时禁用（代码已注释保留，开发中）**：E05（Gen2 过电压 AVR 故障）；E06（强行非同期合闸）
